...
<h1 class="text-center text-sm italic">{{ $donation->pengambilan }}</h1>
<div class="mt-3 flex gap-3 w-max">
    ...
    <div class="w-max bg-navy-500 py-2 px-4 text-white rounded-md shadow-md">
        Heroes : {{ $donation->kuota-$donation->sisa }} / {{ $donation->kuota }}
    </div>
</div>
        ...
        <tbody>
            ...
            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                <td class="px-6 py-4 flex justify-center gap-2">
                    <a href="https://wa.me/{{ $item->telepon }}" class="p-2 rounded-md bg-tosca-300 hover:bg-tosca-600">
                        <svg
                    ...
                    <form action="{{ route('hero.destroy', $item->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button onclick="return confirm('Yakin ingin menghapusnya?')" type="submit" class="p-2 rounded-md bg-red-300 hover:bg-red-600">
                            <svg
                                    xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="white"
